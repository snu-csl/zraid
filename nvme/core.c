		...
		bool at_head)
{
	...
	// blk_execute_rq_nowait(disk, rq, at_head, NULL);
	// status = 0;
...
static int nvme_execute_rq_nowait(struct gendisk *disk, struct request *rq,
		bool at_head)
{
	blk_status_t status;
	...
	struct nvme_passthru_cmd *cmd)
{
	struct nvme_ns *ns;
    int ret;
    //ns = nvme_get_ns_from_dev(&bdev->bd_device);
