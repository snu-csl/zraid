...
void inline print_bio_info(struct bio *bio)
{
	if (bio==NULL) {
		// printk("nvme_handle_cqe bio is NULL!!");
		return;
	}
	...
	if (op_is_write(bio_op(bio)))
		...
		strcpy(rw, "READ");
    // printk("5");
	printk("(%d) nvme_handle_cqe err: %d, rw: %s, lba: %lldKB, len: %dKB\n", 
		current->pid, bio->bi_status, rw,
		dev_lba/2, bio->bi_iter.bi_size/1024);
}
